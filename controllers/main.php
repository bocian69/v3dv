    ...
    function page() {
		if (!isset($_SESSION['UserData']['host']) && !isset($_SESSION['UserData']['login'])){
			if (isset($_POST['host']) || isset($_POST['port']) || isset($_POST['login']) || isset($_POST['haslo']) || isset($_POST['db']) || isset($_POST['dbtype'])){
				...
				echo $validate['return']['dbtype'];
				if (class_exists($validate['return']['dbtype'])) {
					$DbConnector = new $validate['return']['dbtype']($this->registry);
					$connect = $DbConnector->connect($validate['return']['host'], 
											  $validate['return']['port'], 
											  $validate['return']['login'], 
											  $validate['return']['haslo'], 
											  $validate['return']['dbname']);
					if ($connect === true){
						$this->registry->pokapoka($DbConnector);
						die;
						$_SESSION['UserData']['host'] = $validate['return']['host'];
						$_SESSION['UserData']['port'] = $validate['return']['port'];
						$_SESSION['UserData']['login'] = $validate['return']['login'];
						$_SESSION['UserData']['return'] = $validate['return']['return'];
						$_SESSION['UserData']['dbtype'] = $validate['return']['dbtype'];
